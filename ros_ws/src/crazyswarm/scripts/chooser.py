...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=os.path.join(os.path.dirname(
            os.path.realpath(__file__)), "../launch/"),
        help="Path to the configuration *.yaml files")
    parser.add_argument(
        "--stm32Fw",
        type=str,
        default=os.path.join(os.path.dirname(os.path.realpath(
            __file__)), "../../../../crazyflie-firmware/cf2.bin"),
        help="Path to cf2.bin")
    parser.add_argument(
        "--nrf51Fw",
        type=str,
        default=os.path.join(os.path.dirname(os.path.realpath(
            __file__)), "../../../../crazyflie2-nrf-firmware/cf2_nrf.bin"),
    ...
    if not os.path.exists(os.path.join(args.configpath, "allCrazyflies.yaml")) or \
        ...
        print("ERROR: Could not find all yaml configuration files in configpath ({}).".format(
            args.configpath))
        exit()
    ...
    def selected_cfs():
        nodes = [node for id, node in allCrazyflies.items()
                 if widgets[id].checked.get()]
        return nodes
    ...
    allCrazyflies = read_by_id(os.path.join(
        args.configpath, "allCrazyflies.yaml"))
    ...
    class CFWidget(Tkinter.Frame):
        def __init__(self, parent, name):
            ...
            self.batteryLabel = Tkinter.Label(
                self, text="", fg="#999999", padx=0, pady=0)
    ...
    def sysOff():
        ...
        for crazyflie in nodes:
            ...
            uri = "radio://0/{}/1M/E7E7E7E7{}".format(crazyflie["channel"], id)
            subprocess.call(
                ["rosrun crazyflie_tools reboot --uri " + uri + " --mode sysoff"], shell=True)
    ...
    def reboot():
        ...
        for crazyflie in nodes:
            ...
            print(crazyflie["id"])
            subprocess.call(
                ["rosrun crazyflie_tools reboot --uri " + uri], shell=True)
    ...
    def flashSTM():
        ...
        for crazyflie in nodes:
            ...
            subprocess.call(["rosrun crazyflie_tools flash --uri " + uri +
                            " --target stm32 --filename " + args.stm32Fw], shell=True)
    ...
    def flashNRF():
        ...
        for crazyflie in nodes:
            ...
            subprocess.call(["rosrun crazyflie_tools flash --uri " + uri +
                            " --target nrf51 --filename " + args.nrf51Fw], shell=True)
    ...
    def checkBattery():
        ...
        for crazyflie in nodes:
            ...
            try:
                if not bigQuad:
                    voltage = subprocess.check_output(
                        ["rosrun crazyflie_tools battery --uri " + uri], shell=True)
                else:
                    voltage = subprocess.check_output(
                        ["rosrun crazyflie_tools battery --uri " + uri + " --external 1"], shell=True)
            ...
            widgets[crazyflie["id"]].batteryLabel.config(
                foreground=color, text=widgetText)
